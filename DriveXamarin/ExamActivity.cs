...
namespace DriveXamarin
{
    ...
    [Activity(Label = "ExamActivity")]
    public class ExamActivity : Activity, IDialogInterfaceOnDismissListener
    {
        ...
        protected override async void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.activity_exam);
            var query = new Dictionary<string, string>
            {
                ["driveCategory"] = $"{ ChoiseCategoryActivity.driveCategory}"
            };
            using var client = new HttpClient();
            var resultJson = await client.GetAsync(QueryHelpers.AddQueryString("http://10.0.2.2:5000/driveapi/Questions/List", query));
            //var resultJson = await client.GetAsync("http://10.0.2.2:5000/driveapi/Questions/List");
            
            List <Question> resultQuestion = JsonConvert.DeserializeObject<List<Question>>(await resultJson.Content.ReadAsStringAsync());
            
            _chosenList = Question.PullXRandomQuestions(resultQuestion, 30);
        ...
        {//Play sound on completion and display results fragment
            ...
            if (_questionCount == 20 & _inCorrectCount == 0)
            {
                quizresults.Show(finishtxn, "Результаты экзамена");
                _points = 3;
            }
            if (_questionCount == 20 & _inCorrectCount == 1)
            {
                
                questionstoask = 25;
                _chosenList.AddRange(Question.PullXRandomQuestions(_chosenList, 5));
                 NextQuestion();
                
            }
            if (_questionCount == 25 & _inCorrectCount == 1)
            {
                quizresults.Show(finishtxn, "Результаты экзамена");
                _points = 2;
            }
            ...
            if (_questionCount == 20 & _inCorrectCount == 2)
            {
                questionstoask = 30;
                _chosenList.AddRange(Question.PullXRandomQuestions(_chosenList, 5));
                NextQuestion();
            ...
            if (_questionCount == 30 & _inCorrectCount == 2)
            {
                quizresults.Show(finishtxn, "Результаты экзамена");
                _points = 1;
            }
            if (_questionCount == 30 & _inCorrectCount > 1)
            {
                quizresults.Show(finishtxn, "Результаты экзамена");
                _points = 0;
            }
